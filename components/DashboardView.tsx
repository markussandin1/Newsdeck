        ...
        {isMobile ? (
          activeColumns.length > 0 ? (
            <div className="relative h-full flex flex-col">
              ...
              <div
                ref={mobileScrollRef}
                className="nd-mobile-columns flex-1"
                onScroll={onMobileScroll}
              >
                {activeColumns.map((column) => (
                  <div key={column.id} className="nd-mobile-col-page flex flex-col">
                    {/* Pull-to-refresh indicator */}
                    <div
                      className="flex items-center justify-center transition-all duration-200 bg-muted"
                    ...
                    >
                      {isRefreshing ? (
                        ...
                        <div className="text-primary text-sm flex items-center gap-2">
                          <span style={{ display: 'inline-block', transform: pullDistance > 60 ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}>↓</span>
                    ...
                    {/* Column content */}
                    <div
                      ref={column.id === activeColumns[activeColumnIndex]?.id ? scrollContainerRef : undefined}
                ...
                <div className="safe-area-bottom pb-4 pt-2 bg-background border-t border-border">
                  <div className="flex items-center justify-center gap-2">
                    {activeColumns.map((_, index) => (
                      ...
                      >
                        <div
                          className={`rounded-full transition-all ${index === activeColumnIndex ? 'bg-primary w-8 h-2' : 'bg-muted-foreground/30 w-2 h-2'}`}
                        />
                      </button>
                    ))}
                  </div>
