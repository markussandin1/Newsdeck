...
// In production, DATABASE_URL comes from GCP Secret Manager
// In development, DATABASE_URL comes from environment variables
...
const dbWrapper = new Proxy(pgDb, {
  get(target, prop) {
    // Check DATABASE_URL at runtime for each operation (development only)
    // In production, secrets are fetched from Secret Manager
